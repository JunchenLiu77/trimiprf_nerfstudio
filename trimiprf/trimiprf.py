...
"""
Implementation of Tri-MipRF (https://github.com/wbhu/Tri-MipRF/)
"""
...
from .trimiprf_field import TriMipRFField


@dataclass
class TriMipRFModelConfig(ModelConfig):
    """Tri-MipRF Model Config"""

    _target: Type = field(default_factory=lambda: TriMipRFModel)
    """target class to instantiate"""
    enable_collider: bool = False
    """Whether to create a scene collider to filter rays."""
    collider_params: Optional[Dict[str, float]] = None
    """Tri-MipRF doesn't use a collider."""
    ...
    geo_feat_dim: int = 15
    """Dimension of geometric feature vectors stored in grid."""
    net_depth_base: int = 2
    """Depth of the base network."""
    net_depth_color: int = 4
    """Depth of the color network."""
    net_width: int = 128
    """Width of the network."""
    occ_grid_resolution: int = 128
    """Resolution of the occupancy grid."""
    occ_threshold: float = 5e-3
    """Threshold for the occupancy grid."""
...
class TriMipRFModel(Model):
    ...
    Args:
        config: Tri-MipRF configuration to instantiate model
    """

    config: TriMipRFModelConfig

    def __init__(self, config: TriMipRFModelConfig, **kwargs) -> None:
    ...
    def populate_modules(self):
        ...
        self.field = TriMipRFField(
            aabb=self.scene_box.aabb,
            n_levels=self.config.n_levels,
            feat_grid_resolution=self.config.feat_grid_resolution,
            occ_grid_resolution=self.config.occ_grid_resolution,
            feat_dim=self.config.feat_dim,
            geo_feat_dim=self.config.geo_feat_dim,
            net_depth_base=self.config.net_depth_base,
    ...
    def get_training_callbacks(
        ...
        def update_occupancy_grid(step: int):
            self.occupancy_grid.update_every_n_steps(
                ...
                * self.config.render_step_size,
                occ_thre=self.config.occ_threshold,
            )
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        if self.config.use_gradient_scaling:
            rgb_loss = self.grad_scaler.scale(rgb_loss)
